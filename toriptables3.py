...
''' % (basename(__file__), self.trans_port, self.virtual_net,
  ...
  def load_iptables_rules(self):
    ...
    def restart_tor():
      ...
      try:
        ...
        if tor_restart == 0:
          print(" {0}".format(
              "[\033[92m+\033[0m] Ağ durumu \033[92m[Aktif]\033[0m"))
          self.get_ip()
  ...
  def get_ip(self):
    print(" {0}".format(
        "[\033[92m*\033[0m] IP aliniyor bekle"))
    retries = 0
    my_public_ip = None
    while retries < 12 and not my_public_ip:
      ...
      try:
        ...
        sleep(5)
        print(" [\033[93m?\033[0m] IP desteği bekleniyor bekle...")
      except ValueError:
        break
    pass
    if not my_public_ip:
    ...
    print(" {0}".format("[\033[92m+\033[0m] Belirlenen ip \033[92m%s\033[0m" % my_public_ip))
